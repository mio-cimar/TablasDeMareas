...
Modifications: Andrey Bonilla
Date: 22/11/2021
Documentación de la primer parte del código (lectura y preparación de datos)
y lectura previa del archivos para obtener los números de las líneas del dato inicial y final
"""
...
DiaJ=365
Input_data= 'Limon.csv'
File_name='Tabla_Mareas_Limon-2022'
Anne='2022'
Site='Puerto Limón'
State='Limón'
Lon='-83.02°W'
...
df = pd.read_csv('mareas_csv/' + Input_data)
...
temp1 = [] #fechas para cada fase lunar
temp1.append(tm.utc_strftime("%Y-%m-%d %H:%S"))

temp2=[] #fechas para cada fase lunar en HORA LOCAL
for jj in range(49):
...
#path = ruta donde estas los archivos csv
#files = lista de todos los archivos encontrados
    
    for file in files:  #file = archivo específico  

        filename = os.path.join(path, file)
        name = filename.rstrip(".csv")
        
        if Input_data in file: #valida la existencia del archivo digitado al inicio
            
            rows_data = [] #datos obtenidos de TODO el csv
            
            with open(filename) as datos: #abre el archivo seleccionado
                data_reader = csv.reader(datos, delimiter = ",")
                
                for row in data_reader: #lee todas las lineas del csv 
                    rows_data.append(row)
                    
            rt = [] #vector de fechas (horario local)
            for i in range(Datoi,Datof): #recorre solo los datos necesarios (el año de datos seleccionado)
                rt.append(datetime.strptime(rows_data[i][0],"%Y-%m-%dT%H:%M:%S+00:00") - timedelta(hours=6)) #convierte todas las fechas del csv a horario local          
            rt = np.flipud(rt) #invierte el orden del vector de fechas para que se ordene de enero a diciembre
            
            rm = [] #vector de valores de mareas                    
            for i in range(Datoi,Datof):
                rm.append(float(rows_data[i][1])) #obtiene los valores de las mareas
            rm = np.flipud(rm) #invierte el orden del vector de datos para que concuerde con el de fechas
            
            #recibe los vectores de fechas (rt = ti) y valores (rm = ma)
            #ordena los datos por días
            def daylist(ti, ma):
                dlist = [[] for i in range(DiaJ)] #vector con "Diaj" (365 / 366) vectores internos
                vlist = [[] for i in range(DiaJ)] #vector con "Diaj" (365 / 366) vectores internos
                
                for a in range(len(ti)):
                    for b in range(len(ti)):
                        #valida que la fecha en la que se encuentra (recorriendo el vector) --> ti[b].date()
                        #sea igual a la fecha inicial --> ti[0] + "a" dias --> timedelta(days=a)
                        #esto para almacenar los valores y dias en vectores por día dentro del vector principal
                        #ver prints de las lineas 139 y 140 
                        if ti[b].date() == (ti[0] + timedelta(days=a)).date(): 
                            dlist[a].append(ti[b]) #agrega las fechas al vector dlist
                            vlist[a].append('{: 5.2f}'.format(ma[b]))#agrega los valores formatedos(2 decimales) al vector vlist
                            
                return dlist, vlist #retorna los nuevos vectores
            
            dias = daylist(rt, rm) #llama a la función daylist
            mar = dias[1] #nuevo vector con los valores formateados
            dias= dias[0] #nuevo vector de días "confirmados"
            ...
            for i in range(DiaJ):
                for j in range(4-len(dias[i])):
                    dias[i].append("") #agrega un espacio en blanco al final de cada vector de fechas por dia
                    mar[i].append("") #agrega un espacio en blanco al final de cada vector de valores por dia
                
            #recibe los vectores de fechas por dia (dias = di) y valores (mar = ma) 
            ...
            def monlist(di,ma):
                ...
                for m in range(1,13):
                    for x in range(len(di)):
                        ...
                        #los datos previamente ordenados por días los ordena por meses,
                        #es decir, toma todos los vectores donde el mes sea "m"
                        #y los une en un solo vector del mes específico
                        #ver prints de las lineas 171 y 172
                        if di[x][0].month == m:
                            dlist[m-1].append(di[x])    
                            vlist[m-1].append(ma[x])   
                return dlist, vlist
                  
            meses = monlist(dias, mar) #llama a la función monlist
            mar_m = meses[1]
            meses= meses[0]
            ...
            re = ["" for i in range(4)] #prepara un vector vacío de 4 campos

            for i in range(12): #agrega 4 espacios vacios al final del vector
                if len(meses[i]) == 31: 
                    mar_m[i].append(re)
                    meses[i].append(re)
                else:
